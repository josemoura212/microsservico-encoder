...
impl VideoUpload {
    ...
    pub async fn upload_object(&self, object_path: &str, client: &Client) -> anyhow::Result<()> {
        let local_storage_path =
            env::var("localStoragePath").unwrap_or_else(|_| "/tmp".to_string());

        let relative_path = object_path
            .strip_prefix(&format!("{}/", local_storage_path))
            .unwrap_or(object_path);

        let data = fs::read(object_path).await?;
        let upload_type = UploadType::Simple(Media::new(relative_path.to_string()));
        ...
        tracing::info!("Uploaded {} to {}", relative_path, self.output_bucket);
    ...
    pub async fn process_upload(
        ...
        for path in &self.paths {
            ...
            let storage_path = local_storage_path.clone();
